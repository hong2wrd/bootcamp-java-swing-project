...
public class ShelterDetailPage {
	...
	protected RoundButton modShelBtn;
	protected RoundButton deleteShelBtn;
	protected RoundButton cancelBtn;
	
	private ShelterDAO shelDAO = new ShelterDAO();
	
	
	public ShelterDetailPage() {
...
//	    frame.setContentPane(mainPanel);
		...
		if(MainFrame.loginToken==0) {
			...
			deleteShelBtn = new RoundButton("삭제하기");
			deleteShelBtn.setBounds(360, 520, 100, 35);
			deleteShelBtn.setBackground(new Color(216, 33, 72));
			deleteShelBtn.setRoundButton(10, 10);
			deleteShelBtn.setFont(new Font("강원교육모두 Bold", Font.PLAIN, 20));
			deleteShelBtn.setForeground(new Color(255, 255, 255));
			deleteShelBtn.addActionListener(new ActionListener() {
				
				@Override
				public void actionPerformed(ActionEvent e) {
					int res = shelDAO.deleteShelter(MainFrame.selectedShel.getShelCode());
					if(res>0) {
						MainFrame.deleteDial.setVisible(true);
						
						MainFrame.shelterPage = new ShelterPage();
						MainFrame.mainPanel.add(MainFrame.shelterPage.mainPanel, "shelterPanel");
						MainFrame.card.show(MainFrame.mainPanel, "shelterPanel");
					}else {
						MainFrame.errorDial.setVisible(true);
					}
				}
			});
			joinPanel.add(deleteShelBtn);

			modShelBtn = new RoundButton("수정하기");
			modShelBtn.setBounds(140, 520, 100, 35);
			modShelBtn.setBackground(new Color(205, 194, 174));
			modShelBtn.setRoundButton(10, 10);
			modShelBtn.setFont(new Font("강원교육모두 Bold", Font.PLAIN, 20));
			modShelBtn.setForeground(new Color(53, 66, 89));
			modShelBtn.addActionListener(new ActionListener() {
				
				@Override
				public void actionPerformed(ActionEvent e) {
					
					String shelName = nameField.getText();
					String shelAddr = addrField.getText();
					String shelPh1 = phoneField1.getText();
					String shelPh2 = phoneField2.getText();
					String shelPh3 = phoneField3.getText();
					
					if(shelName.isEmpty()||shelAddr.isEmpty()||shelPh1.isEmpty()||shelPh2.isEmpty()||shelPh3.isEmpty()) {
						MainFrame.dataRequestDial.setVisible(true);
					}else {
						ShelterDTO shel = new ShelterDTO();
						shel.setShelCode(MainFrame.selectedShel.getShelCode());
						shel.setShelName(shelName);
						shel.setShelAddr(shelAddr);
						shel.setShelPh1(shelPh1);
						shel.setShelPh2(shelPh2);
						shel.setShelPh3(shelPh3);
						
						int res = shelDAO.updateShelter(shel);
